...
export function TransactionModal({visible,initial,autoScan,onClose,onSave}){
  ...
  return <ModalShell visible={visible} title={initial?'Izmeni transakciju':'Nova transakcija'} onClose={onClose}>
    ...
    {type==='expense'&&qrCategorySuggestion?<View style={s.qrCategoryCard}>
      ...
      <Text style={s.qrCategoryNormalization}>SR normalizacija: ćirilica i latinica se analiziraju u istoj internoj formi. Klasifikator prepoznaje i nepoznate/lokalne apoteke preko naziva prodajnog mesta i sadržaja računa, kao i klinike, drogerije, gorivo, servise, restorane, tehniku i druge tipove troškova.</Text>
